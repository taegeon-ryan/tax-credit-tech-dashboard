...
export default function TechDetail({ data, tech, sector, onBack }) {
  ...
  const facilities = useMemo(() => {
    return data[facilityKey]
      .filter((r) => r.current && r.status !== '삭제' && r.tech_name === tech.tech_name)
    ...
    <div className="drill-view">
      ...
      <div className="detail-section">
        ...
        <div className="detail-meta">
          ...
          <span>적용일: {tech.apply_date?.slice(0, 7)}</span>
        </div>
      </div>

      <div className="detail-section">
        <h4 className="detail-heading">
          매칭 시설 <span className="detail-count">{facilities.length}건</span>
        </h4>
        {facilities.length === 0 ? (
          <div className="empty-msg small">이 기술에 매칭되는 현행 시설이 없습니다.</div>
        ) : (
          <ul className="facility-list">
            {facilities.map((f, i) => (
              <li key={i} className="facility-item">
                <p className="facility-desc">{f.facility_description}</p>
                <span className="facility-meta">적용일 {f.apply_date?.slice(0, 7)}</span>
              </li>
            ))}
          </ul>
        )}
